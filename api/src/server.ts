...
// Postgres pool
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || 'postgresql://support:support@db:5432/adtech'
});

fastify.get('/api/ad', async (request, reply) => {
  return { image: 'https://via.placeholder.com/300x250?text=Ad+Creative', destination: 'https://www.google.com', ad_id: 'ad-001' };
});

// Click tracking endpoint
fastify.post('/api/click', async (request, reply) => {
  const { ad_id, user_agent, ip_address } = request.body as { ad_id?: string; user_agent?: string; ip_address?: string };
  ...
  const ip = ip_address || request.ip || request.headers['x-forwarded-for'] || 'unknown';
